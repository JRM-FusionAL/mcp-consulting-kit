...
if (-not $SkipRemote) {
    ...
    $cmd = "echo -n '  port 8101: '; curl -fsS --max-time 5 http://127.0.0.1:8101/health >/dev/null && echo OK || echo DOWN; echo -n '  port 8102: '; curl -fsS --max-time 5 http://127.0.0.1:8102/health >/dev/null && echo OK || echo DOWN; echo -n '  port 8103: '; curl -fsS --max-time 5 http://127.0.0.1:8103/health >/dev/null && echo OK || echo DOWN; if curl -fsS --max-time 5 http://127.0.0.1:8089/health >/dev/null; then echo '  FusionAL: OK on port 8089'; elif curl -fsS --max-time 5 http://127.0.0.1:8009/health >/dev/null; then echo '  FusionAL: OK on port 8009'; else echo '  FusionAL: DOWN (checked 8089 and 8009)'; fi"
